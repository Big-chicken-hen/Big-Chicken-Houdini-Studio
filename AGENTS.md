...
The latest scope and acceptance decision is `docs/tc1-brief.md`: upgrade current-installation discovery and bounded working-set observation on `codex/tool-capability-discovery`. PR #5 merged at `1e9f0f4`; its closure record is `docs/pr5-closure-brief.md`.
...
- TC-1 changes only context/inspect/lookup, related descriptions, validation and bounded result summaries. Keep seven tools, existing execution/receipt/capture boundaries, and no UI, knowledge expansion or Thread requirements. Use the complete paired two-turn authoring acceptance in `docs/tc1-brief.md`; merge requires the stated behavior/quality evidence and Pro review.
