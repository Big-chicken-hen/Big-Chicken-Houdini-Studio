...
The [latest TC-1 decision](tc1-brief.md) authorizes current-installation discovery and bounded working-set observation on `codex/tool-capability-discovery`, based on current main. Upgrade the existing context/inspect/lookup tools, schema validation and actionable result summaries. Keep execution/receipt/capture semantics, seven tools and the UI unchanged. Thread requirements remain deferred. TC-1 requires complete paired two-turn authoring evidence and the approved behavior/quality gate before Pro review and merge.
