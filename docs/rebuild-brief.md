...
The [latest TC-2 decision](tc2-brief.md) authorizes targeted execution feedback and SOP/OBJ review capture on `codex/tool-capability-feedback`. Reuse TC-1 inspection/results and keep seven tools, the existing queue/receipt/artifact boundaries and the UI. PR #6 can technically merge after its address-summary correction and candidate CI; its original model comparison remains [TC1-A1](deferred-acceptance.md), using a frozen TC-1 candidate. Technical integration does not establish model benefit. TC-2 must pass real H22 GUI framing/frame/view/camera restoration checks before technical merge; model evidence may remain explicitly pending on external login. Thread requirements remain deferred.
