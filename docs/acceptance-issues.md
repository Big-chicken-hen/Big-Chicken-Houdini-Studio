...
## R4-CONN-1：运行期间 10048 与发送恢复缺口

- 2026-09-16 用户报告：运行一段时间后，发送或补充消息时 Panel 出现 WinError 10048。
- [Pro 审查](r4-connection-review.md) 允许对取证、明确未发送的恢复及必要退避小范围解冻；功能范围继续冻结。
- 现场系统 Tcpip 4231／4227 原始事件已核对，但截图失败的路由、阶段和连接来源仍未关联，不能认定 Studio 或 Clash 导致端口耗尽。
- 隔离测试复现旧源码的两个 unknown 分类和一个被后台连接代次屏蔽的发送终态；修正后的原文、附件保留和人工重提路径通过。
- 本地 42 项定向检查及 Ruff 通过。实际新包的断连恢复、覆盖既往使用时长的真实 GUI 检查、现场关联取证仍待完成；不能作为本问题已关闭或最终发布通过。
- 后续 CI `35046247959` 的四个 backend 作业通过，native-ui 的旧删除测试因在 `panel.call` 而非当前发送使用的 Api 层拦截，未捕获 `/turn`，在 140 项中产生一个 error；渲染步骤跳过。按用户提供的新审查仅适配测试并使用身份匹配的合法接纳回应，增加未删除正向对照；本地完整 UI 141 项及 Ruff 通过。新提交的完整 CI/渲染与独立新包验证继续分别记录，不回写旧包为新包验收。
- `145c434` 的 CI `35047403865` 五个作业及全部离屏渲染通过，随后构建独立验收包 `0.1.0-rc.1-145c434d0b9c`；245 项载荷校验、包内 24 项隔离回归和五文件 ZIP 读回通过。包内测试首次缺 QtTest 的失败已保留，后续仅在测试目录加载同锁定 wheel 的测试模块，安装载荷未增加该依赖。旧外测 ZIP 哈希不变；真实新包与标准用户流程仍 pending。
- 用户随后在 E 盘日常真实环境复测后反馈“没问题了！！提交吧！！”。新会话 PID 21088 的模块身份指向日常 E 盘源码；限时日志有 30.075 秒窗口、79 次请求完成、0 次失败。该窗口没有 turn/steer 条目，使用时长和分项结果未逐一报告，故记录用户本轮使用正常，不将其扩展为长时、独立新包或标准用户验收。详见验证记录中的 Owner 复测及机器可读摘要。
- 用户随后明确不需要独立验收包。已删除 `145c434` 的本机 ZIP、安装器和解包副本，并清理闲置测试依赖/缓存，共释放约 1.21 GiB；产品源码、日常运行依赖和真实用户数据未改动。历史构建/测试证据保留，后续不默认重建独立包。`72f9b11` 的 CI `35050005941` 五个作业全部通过。
- 2026-09-17 用户明确要求更新安装包、暂不实施 Mermaid，并将包内说明收拢为单一 README。已从 `b6bed7c` 更新当前交付：245 项清单、包内 24 项隔离回归通过；交付 ZIP 仅包含 Installer.exe 和 README.md。新包包含连接恢复修正并替换原 `e6beb5a` ZIP。临时构建副本已清理，真实新包与标准用户安装验收仍未执行。详见 [交付记录](r4-package-refresh-20260917.md)。
- 标准用户安装流程仍是独立待验收项；R4-HELP-1 的关闭结论保持，现场 10048 的具体来源未由打包证明。详见 [验证记录](r4-connection-validation.md)。
