...
class IconTests(unittest.TestCase):
    ...
    def test_missing_resources_keep_the_same_button_action_and_readable_text(self):
        from studio.ui.theme import apply_theme
        font = Path("C:/Windows/Fonts/msyh.ttc")
        if font.is_file():
            QtGui.QFontDatabase.addApplicationFont(str(font))
        root = QtWidgets.QWidget()
        root.setObjectName("studioFallbackFixture")
        root.setFont(QtGui.QFont("Microsoft YaHei UI", 11))
        layout = QtWidgets.QVBoxLayout(root)
        button = QtWidgets.QPushButton("发送下一条消息")
        layout.addWidget(button)
        apply_theme(root)
        calls = []
        button.clicked.connect(lambda: calls.append("send"))
        with patch.object(icons, "_svg", side_effect=FileNotFoundError()):
            icons.set_button_icon(button, "arrow-up", text="发送下一条消息", fallback_text="发送", icon_only=True)
            root.show()
            button.setFixedSize(36, 36)
            self.app.processEvents()
            self.assertTrue(button.icon().isNull())
            self.assertEqual(button.text(), "发送")
            self.assertEqual(button.accessibleName(), "发送下一条消息")
            self.assertEqual(button.toolTip(), "发送下一条消息")
            self.assertEqual(button.size(), QtCore.QSize(36, 36))
            self.assertEqual(button.font().pointSizeF(), 11)
            option = QtWidgets.QStyleOptionButton()
            option.initFrom(button)
            content = button.style().subElementRect(QtWidgets.QStyle.SE_PushButtonContents, option, button)
            self.assertLessEqual(button.fontMetrics().horizontalAdvance(button.text()), content.width())
            self.assertLessEqual(button.fontMetrics().height(), content.height())
            button.click()
            self.assertEqual(calls, ["send"])
            self.assertEqual(icons.icon_diagnostics()[0]["code"], "ICON_RESOURCE_MISSING")
        icons.set_button_icon(button, "not-approved", text="已有动作", fallback_text="动作", icon_only=True)
        self.assertTrue(button.icon().isNull())
